/**
 * file name   : src/batch/scrap/baemin/batch.js
 * description : 배달의민족 배치
...
const scrapRun = async ({ BATCH_HISTORY_SEQ, BATCH_STATUS_LISTENER}) => {
    ...
    do{
        ...
        // 사업자 번호별 스크래핑 진행
        for(let { SCRP_USER_ACCOUNT_SEQS, ID_LOGIN, PW_LOGIN } of targets){
                ...
                for(let SCRP_USER_ACCOUNT_SEQ of SCRP_USER_ACCOUNT_SEQS){
                    ...
                    let CD_TRANforScrapModules = userScrapInfos.reduce((prev, userScrapInfo) => {
                        ...
                        switch(userScrapInfo.CD_TRAN){
                            case "baemin_o" :   module = scrapOrderHistoryAsync;        break;  // 주문내역
                            case "baemin_p" :   module = scrapSettleHistoryAsync;       break;  // 정산내역
                            default : return prev;
                        }
